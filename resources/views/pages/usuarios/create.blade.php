...
@section('content')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Usuário cadastrar</h1>
    </div>

    <form class="form" action="{{ route('usuario.cadastrar') }}" method="POST">
        @csrf
        <div class="input-group mb-3">
            <span class="input-group-text" id="basic-addon1">Nome</span>
            <input type="text" class="form-control @error('name') is-invalid @enderror" value="{{ old('name') }}"
                name="name" placeholder="Nome do usuário" aria-label="Nome do usuário" aria-describedby="basic-addon1">
            @if ($errors->has('name'))
        ...
        <div class="input-group mb-3">
            ...
            <input type="text" class="form-control @error('email') is-invalid @enderror" value="{{ old('email') }}"
                name="email" placeholder="E-mail do usuário" aria-label="E-mail do usuário" aria-describedby="basic-addon1">
            @if ($errors->has('email'))
        ...
        <div class="input-group mb-3">
            ...
            <input type="password" class="form-control @error('password') is-invalid @enderror" value="{{ old('password') }}"
                name="password" placeholder="Senha do usuário" aria-label="Senha do usuário" aria-describedby="basic-addon1">
